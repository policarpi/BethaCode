...
SELECT descricao, tipo, valor_evento AS Evento_Superior_A_100 FROM eventos
	WHERE valor_evento > 100 AND tipo = 'A'
...
SELECT mes_competencia AS Competencia, count (*) AS Ocorrencias,
	SUM (valor_bruto) AS ValorLiquido,
	SUM (valor_liquido) AS ValorLiquido
		FROM  calculos
...
SELECT * FROM funcionarios
		WHERE EXISTS (SELECT 1 FROM calculos
					  ...
					  AND calculos.ano_competencia = 2021)
...
SELECT  DISTINCT nome FROM funcionarios, eventos
	WHERE i_funcionarios IN (SELECT i_funcionarios FROM eventos_manuais
							 WHERE eventos_manuais.mes = 7 AND
							eventos_manuais.anos = 2021)
							AND eventos.tipo = 'D'
